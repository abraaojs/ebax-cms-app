...
import LinesLeftContainer from "./LinesLeftContainer"

const Line = (props: any) => {
  const [containerOpen, setContainer] = useState(false)
  const { line, columnsLeft } = props

  return (
    <div className="bg-white pt-[16px] py-[20px] px-[16px] rounded-[10px] mt-[16px]">
      <div className="flex justify-between ">
        <span className="paragraph-3 font-700">{line.title}</span>
      ...
      <LinesLeftContainer line={line} columnsLeft={columnsLeft} containerOpen={containerOpen} />
      <ButtonClear blok={line.button[0]} uppercase={true} className="paragraph-3" link="" />
    </div>
  )
}
